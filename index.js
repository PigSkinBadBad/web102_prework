...
const individualContributions = GAMES_JSON.reduce((total, game) => total + game.backers, 0);
...
const unfundedGamesNumber = GAMES_JSON.filter(game => game.pledged < game.goal).length;

// create a string that explains the number of unfunded games using the ternary operator
const unfundedString = `A total of $${GAMES_JSON.reduce((total, game) => total + game.pledged, 0).toLocaleString()} has been raised for ${GAMES_JSON.length} games. Currently, ${unfundedGamesNumber} game${unfundedGamesNumber === 1 ? " remains" : "s remain"} unfunded. We need your help to fund these amazing games!`;
